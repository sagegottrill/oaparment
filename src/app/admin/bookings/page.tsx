            ...
            {bookings.map((booking) => (
              <tr key={booking.id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                ...
                <td style={{ padding: 'var(--spacing-sm)' }}>
                  <span style={{ 
                    color: booking.status === 'Confirmed' ? 'var(--color-success)' : 
                           booking.status === 'Pending' ? '#F59E0B' : 'var(--color-error)'
                  }}>
                    {booking.status}
                  </span>
                </td>
                <td style={{ padding: 'var(--spacing-sm)' }}>
                  <div className="flex gap-sm">
                    {booking.status === 'Pending' && (
                      <button className="btn btn-primary" style={{ padding: 'var(--spacing-xs) var(--spacing-sm)', fontSize: '0.875rem' }}>Confirm</button>
                    )}
                    {booking.status !== 'Cancelled' && (
                      <button className="btn btn-outline" style={{ padding: 'var(--spacing-xs) var(--spacing-sm)', fontSize: '0.875rem', color: 'var(--color-error)', borderColor: 'var(--color-error)' }}>Cancel</button>
                    )}
                  </div>
                </td>
              </tr>
