export default function AdminRoomsPage() {
  ...
  return (
    <div>
      <div className="flex justify-between items-center gap-md" style={{ marginBottom: "var(--spacing-xl)", flexWrap: "wrap" }}>
              ...
              {rooms.map((room) => (
                <tr key={room.id} style={{ borderBottom: "1px solid var(--color-border)" }}>
                  ...
                  <td style={{ padding: "var(--spacing-sm)" }}>
                    <span style={{ color: room.status === "Active" ? "var(--color-success)" : "#F59E0B" }}>{room.status}</span>
                  </td>
                  <td style={{ padding: "var(--spacing-sm)" }}>
                    <button className="btn btn-outline" style={{ padding: "var(--spacing-xs) var(--spacing-sm)", fontSize: "0.875rem" }}>
                      Edit
                    </button>
                  </td>
                </tr>
